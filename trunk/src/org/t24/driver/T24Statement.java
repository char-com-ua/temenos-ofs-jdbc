 ...
 * @see ResultSet 
 */
public class T24Statement implements Statement {

    protected T24Connection con = null;
    protected T24ResultSet rs = null;
    protected boolean moreRs = false;
    protected String preparedOfs = null;
    protected String postProcessSql = null;
    public static final String POSTFORMATING = "-POSTFORMATING-";
    
  	public final static int QUERY_TYPE_ENQ		=1;
	public final static int QUERY_TYPE_ENQ_FMT	=2;
	public final static int QUERY_TYPE_FMT		=3;
	public int queryType = 0;

    T24QueryFormatter queryFormatter;

    protected T24Statement(T24Connection con) {
        this.con = con;
    ...
    /**
     * prepares internal ofs and postprocessing variables
     */
    protected void prepare(String sql) throws SQLException {
		if ( sql.matches("\\s*SELECT\\s*ENQUIRY\\.SELECT.*") ){
			queryType=QUERY_TYPE_ENQ;
		}else if(Pattern.compile("\\s*SELECT\\s+FORMATTER\\s*ENQUIRY\\.SELECT.*",Pattern.DOTALL).matcher(sql).matches()){
			queryType=QUERY_TYPE_ENQ_FMT;
		}else if(Pattern.compile("\\s*SELECT\\s+FORMATTER\\s.*",Pattern.DOTALL).matcher(sql).matches()){
			queryType=QUERY_TYPE_FMT;
		}else throw new T24Exception("Not supported query type: "+sql);
        
        sql = sql.trim();
        //for data services version v2.3
        //sql type detected by sql prefix (for example SELECT)
        //and only select exceptions processed correctly
        //so in dataservice you can add "SELECT " before ofs
        //and here remove it.
        if (sql.substring(0, 7).equalsIgnoreCase("SELECT ")) {
            sql = sql.substring(7).trim();
        }
        if (sql.substring(0, 10).equalsIgnoreCase("FORMATTER ")) {
            sql = sql.substring(10).trim();
        }
        sql = sql.replaceAll("\\$\\{USER\\}", con.tcUser);
        sql = sql.replaceAll("\\$\\{PASS\\}", con.tcPass);

        int postProcesIndex = sql.indexOf(POSTFORMATING);
        if (postProcesIndex == -1) {
            postProcessSql = "";
        } else {
            postProcessSql = sql.substring(postProcesIndex + POSTFORMATING.length()).trim();
            sql = sql.substring(0, postProcesIndex).trim();
        }

        this.preparedOfs = sql;
    }
    ...
    public boolean execute(String sql) throws SQLException {
        //prepare ofs from sql
        prepare(sql);
        //exec ofs. 
        String ofsResp = con.t24Send(preparedOfs);
        //create resultset from responce
        rs = new T24ResultSet(this, preparedOfs, ofsResp);

        for (int i = 1; i <= rs.getRowCount(); i++) {
            rs.setValue(i, "rowNum", ""+i);
            HashMap<String, String> hm = queryFormatter.postProcesing((T24ResultSetMetaData) rs.getMetaData(), rs.getDataRow(i), postProcessSql);            
            for (String columnName : hm.keySet()) {
                String columnValue = hm.get(columnName);
                rs.setValue(i, columnName, columnValue);
            }            
        }
        moreRs = true;
        //resultset always present, so return true
        return true;
    }
     ...
     *        <code>Statement.KEEP_CURRENT_RESULT</code> or
     *        <code>Statement.CLOSE_ALL_RESULTS</code>
     *@exception T24FeatureNotSupportedException if 
